...
export class AutoBin implements PreparedBuildUpdate {
	...
	update(packageJson: PackageJson, format: InternalModuleFormat) {
		...
		const packageJsonUpdates = Object.entries(this.entryMap).reduce(
			(result, [key, value]) => {
				...
				binFile = `${fileName}.${getBundledFileExtension(format, packageJson.type)}`;
				this.outputFilesByFormat[format].push(binFile);
	...
	async adjustPaths(
		...
		console.log('this.outputFilesByFormat', this.outputFilesByFormat);
		await makeJavascriptFilesExecutable(
			this.outputFilesByFormat[format].map((path) => enterPathPosix(path, 1)),
			{
				cwd: join(this.options.cwd, this.options.outDir),
				logger: this.options.logger,
				format,
				packageJsonType: packageJson.type ?? 'commonjs',
			}
		);
	...
	private async preLink(binRecord: Record<string, string>): Promise<void> {
		...
		const packageBinDirectoryPath = join(this.options.cwd, 'node_modules', '.bin');
		...
		console.log('symlinksToMake', symlinksToMake);
